...
%cholesky con fattori r ottimi e p ottimo del favar
...
[var_irf,~,var_irf_boot,~,~,~] = CholeskyIdentification(Y,optimal_p,H,constant,n_bootstrap,cum_index,var_labels,display_on);

%cholesky con favar baseline
favar_baseline_labels=[1:r_baseline,"Policy Rate"];
[favar_baseline_irf,~,favar_baseline_irf_boot,~,~,~] = CholeskyIdentification(Z_baseline,optimal_p,H,constant,n_bootstrap,cum_index,favar_baseline_labels,display_on);

%cholesky con shadowrate
favar_baseline_shadow_labels=[1:r_baseline,"Shadow Rate"];
[favar_baseline_shadow_irf,~,favar_baseline_shadow_irf_boot,~,~,~] = CholeskyIdentification(Z_baseline_shadowrate,optimal_p,H,constant,n_bootstrap,cum_index,favar_baseline_shadow_labels,display_on);

%cholesky con shadowrate con hicp e gdp espliciti
favar_shadow_labels=[1:r,"GDP","HICP","Shadow Rate"];
[favar_shadow_irf,~,favar_shadow_irf_boot,~,~,~] = CholeskyIdentification(Z_shadowrate,optimal_p,H,constant,n_bootstrap,cum_index,favar_shadow_labels,display_on);
...
%% Calcola l'R^2 medio della regressione X su Z
%Lambda_test = (Z \ X_std)'; % (114 x 6)
%Residui = X_std - (Z * Lambda_test');
%Var_Tot = var(X_std);
%Var_Res = var(Residui);
%R2 = 1 - (Var_Res ./ Var_Tot);

%figure; histogram(R2); title('Distribuzione R^2 dei Fattori sulle Variabili X');
%fprintf('R^2 Medio: %.4f\n', mean(R2));